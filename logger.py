'''
Ref: https://github.com/allenai/allennlp/blob/main/allennlp/common/logging.py
'''
...
class BiencoderExperimentLogger(logging.Logger):
    """
...
logging.setLoggerClass(BiencoderExperimentLogger)
...
def prepare_global_logging(
    experiment_dir: Union[str, PathLike],
    rank: int = 0,
    world_size: int = 1,
) -> None:
    ...
    # create handlers
    if world_size == 1:
        log_file = os.path.join(experiment_dir, "out.log")
        formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(name)s - %(message)s")
    else:
        log_file = os.path.join(experiment_dir, f"out_worker{rank}.log")
