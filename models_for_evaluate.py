...
class EntityIterator(Model):
    ...
    def forward(self, gold_dui_canonical_and_def_concatenated, entity_uniq_id):
        batch_num = gold_dui_canonical_and_def_concatenated['tokens']['token_ids'].size(0)
        encoded_entites = self._candidate_entities_emb_returner(batch_num, gold_dui_canonical_and_def_concatenated)
        output = {'encoded_entites': encoded_entites,
                  'entity_uniq_id': entity_uniq_id}
        return output
